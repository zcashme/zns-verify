...
pub fn parse_memo(raw: &[u8]) -> Result<ParsedMemo<'_>, MemoError> {
    ...
    match verb {
        "claim" => {
            Ok(ParsedMemo::Lifecycle { action: Action::Claim, name, ua: required(arg)?, prev_rcm })
        }
        "update" => {
            Ok(ParsedMemo::Lifecycle { action: Action::Update, name, ua: required(arg)?, prev_rcm })
        }
        "release" => match (arg, prev_rcm) {
            // Request form: exactly three fields.
            (None, None) => {
                Ok(ParsedMemo::Lifecycle { action: Action::Release, name, ua: "", prev_rcm: None })
            }
            // Name Note form: positional empty `ua`, then the witness.
            (Some(""), Some(_)) => {
                Ok(ParsedMemo::Lifecycle { action: Action::Release, name, ua: "", prev_rcm })
            }
            _ => Err(MemoError::FieldCount),
        },
        "challenge" if prev_rcm.is_none() => {
            Ok(ParsedMemo::Challenge { name, nonce: required(arg)? })
        }
        "confirm" if prev_rcm.is_none() => Ok(ParsedMemo::Confirm { name, nonce: required(arg)? }),
...
mod tests {
    ...
    fn lifecycle<'a>(
        ...
        ParsedMemo::Lifecycle { action, name, ua, prev_rcm }
    }
    ...
    fn parses_request_forms() {
        ...
        assert_eq!(
            ...
            Ok(ParsedMemo::Challenge { name: "alice", nonce: "deadbeef" }),
        );
        assert_eq!(
            parse_memo(b"ZNS:confirm:alice:deadbeef"),
            Ok(ParsedMemo::Confirm { name: "alice", nonce: "deadbeef" }),
        );
    }
    ...
    fn parses_name_note_forms() {
        ...
        assert_eq!(parse_memo(m.as_bytes()), Ok(lifecycle(Action::Release, "alice", "", Some(want))));

        // The witness must be exactly 64 lowercase hex chars.
        assert_eq!(parse_memo(b"ZNS:claim:alice:u1xxx:abcd"), Err(MemoError::InvalidPrevRcm));
    ...
    fn strict_field_counts() {
        ...
        assert_eq!(parse_memo(b"ZNS:update:alice:u1x:extra"), Err(MemoError::InvalidPrevRcm));
        assert_eq!(parse_memo(b"ZNS:release:alice:junk"), Err(MemoError::FieldCount));
        assert_eq!(parse_memo(b"ZNS:release:alice:"), Err(MemoError::FieldCount));
        ...
        assert_eq!(parse_memo(b"ZNS:settle:alice:u1x"), Err(MemoError::UnknownVerb));
    }
    ...
    fn name_rule_is_dns_label() {
        ...
        assert_eq!(parse_memo(b"ZNS:claim:Alice:u1x"), Err(MemoError::InvalidName));
    }

    #[test]
    fn encode_round_trips() {
        let m = encode_request(Action::Claim, "alice", "u1xxx").unwrap();
        assert_eq!(parse_memo(&m), Ok(lifecycle(Action::Claim, "alice", "u1xxx", None)));
        let m = encode_request(Action::Release, "alice", "").unwrap();
        assert_eq!(parse_memo(&m), Ok(lifecycle(Action::Release, "alice", "", None)));

        let prev = [0xa5u8; 32];
        let m = encode_name_note(Action::Update, "alice", "u1new", &prev).unwrap();
        assert_eq!(parse_memo(&m), Ok(lifecycle(Action::Update, "alice", "u1new", Some(prev))));
        let m = encode_name_note(Action::Release, "alice", "", &prev).unwrap();
        assert_eq!(parse_memo(&m), Ok(lifecycle(Action::Release, "alice", "", Some(prev))));

        let m = encode_challenge("alice", "deadbeef").unwrap();
        assert_eq!(parse_memo(&m), Ok(ParsedMemo::Challenge { name: "alice", nonce: "deadbeef" }));
        let m = encode_confirm("alice", "deadbeef").unwrap();
        assert_eq!(parse_memo(&m), Ok(ParsedMemo::Confirm { name: "alice", nonce: "deadbeef" }));
    }

    #[test]
    fn encode_rejects_what_parse_rejects() {
        assert_eq!(encode_request(Action::Claim, "alice", ""), Err(MemoError::EmptyArg));
        assert_eq!(encode_request(Action::Release, "alice", "u1x"), Err(MemoError::FieldCount));
        assert_eq!(encode_request(Action::Claim, "Alice", "u1x"), Err(MemoError::InvalidName));
        assert_eq!(
