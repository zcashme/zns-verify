...
#[test]
fn deterministic() {
    let a = zns_psi_rcm(b"claim", b"alice", b"u1xxx", b"none", &[0u8; 32]);
    let b = zns_psi_rcm(b"claim", b"alice", b"u1xxx", b"none", &[0u8; 32]);
    assert_eq!(a.0, b.0);
    assert_eq!(a.1, b.1);
}
...
fn length_prefix_prevents_collision() {
    ...
    // distinguishes these.
    let a = zns_psi_rcm(b"claim", b"ali", b"cebob", b"none", &[0u8; 32]);
    let b = zns_psi_rcm(b"claim", b"alice", b":bob", b"none", &[0u8; 32]);
    assert_ne!(a.0, b.0);
    assert_ne!(a.1, b.1);
}
...
fn prev_rcm_and_action_change_output() {
    let base = zns_psi_rcm(b"claim", b"alice", b"u1xxx", b"none", &[0u8; 32]);
    let other_prev = zns_psi_rcm(b"claim", b"alice", b"u1xxx", b"none", &[1u8; 32]);
    assert_ne!(base.0, other_prev.0);
    assert_ne!(base.1, other_prev.1);

    let update = zns_psi_rcm(b"update", b"alice", b"u1xxx", b"none", &[0u8; 32]);
